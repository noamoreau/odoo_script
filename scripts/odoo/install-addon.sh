...
do
  if [[ $? == 0 ]]
  then
      ssh odoo1 wget "https://apps.odoo.com/loempia/download/$addon/${version}.0/$addon.zip -P "'$HOME/'"$1/addons"
      ssh odoo1 'unzip $HOME/'"$1/addons/$addon.zip -d "'$HOME'"/$1/addons/"
  else
    echo -e "${rouge_fonce}Le addon $addon n'est pas valide, veillez relancer le script en entrant un nom valide${reset}"  
  fi
done

ssh odoo1 "cd "'$HOME'"/$1 && docker-compose up -d"
